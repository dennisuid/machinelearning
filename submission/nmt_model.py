...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, self.hidden_size, num_layers=1,
                               bidirectional=True, bias=True, dropout=dropout_rate)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hides, (h_n, c_n) = self.encoder(packed_X)
        enc_hiddens, output_lengths = pad_packed_sequence(enc_hides, batch_first=True)
        first_h_n_row = h_n[0, :, :]
        second_h_n_row = h_n[1, :, :]
        h_project = torch.cat((first_h_n_row, second_h_n_row), 1)
        init_decoder_hidden = self.h_projection(h_project)
        first_c_n_row = c_n[0, :, :]
        second_c_n_row = c_n[1, :, :]
        c_project = torch.cat((first_c_n_row, second_c_n_row), 1)
        init_decoder_cell = self.c_projection(c_project)
